...
def get_session_data(request: Request) -> SessionData:
    # Test-mode override: allow header-driven auth in CI/testing
    if security_settings.security_test_mode:
        test_user = request.headers.get("x-test-user")
        test_org = request.headers.get("x-test-org")
        # In test mode, always honor explicit headers to seed session data,
        # even if a middleware pre-populated an anonymous session.
        if test_user:
            # Lazily create a synthetic session
            try:
                user_id = int(test_user)
            except ValueError:
                user_id = None
            try:
                org_id = int(test_org) if test_org else None
            except ValueError:
                org_id = None
            from .utils import SessionData as _SD
